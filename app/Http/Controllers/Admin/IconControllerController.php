...
class IconControllerController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        try {
            ...
            return redirect()->route('icon.index')->with('success', APIResponseMessage::CREATED);
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('icon.index')->with('failed', APIResponseMessage::FAIL);
        }
    }
    ...
    public function show(string $id)
    {
        ...
        $icon = Icon::with([])->find($categoryId);
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            return redirect()->route('icon.index')->with('success', APIResponseMessage::UPDATED);
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('icon.index')->with('error', APIResponseMessage::ERROR_EXCEPTION);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            Icon::with([])->find($id)->delete();

            DB::commit();


            return redirect()->route('icon.index')->with('success', APIResponseMessage::DELETED);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $model = Icon::query()->orderBy('id', 'asc');
        ...
        return DataTables::eloquent($model)
            ...
            ->addColumn('edit', content: function ($data) {
                $edit_url = route('icon.show',encrypt($data->id));
             ...
             ->addColumn('delete', function ($data) {
                return view('admin.icon.partials._delete', compact('data'));
            })
            ->rawColumns(['edit'])
            ->toJson();
    }
